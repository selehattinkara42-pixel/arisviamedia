...
export default function PortfolioSectionClient({ items, showAll = false, content }: { items: PortfolioItem[], showAll?: boolean, content: PortfolioContent }) {
                ...
                <div className="container mx-auto px-6">
                    ...
                    >
                        <h2 className={`font-bold uppercase tracking-[0.3em] text-primary-gold mb-4 ${getSize(content.titleSmall?.fontSize, 'text-xs')}`}>
                            {content.titleSmall?.content || 'Seçkin İşlerimiz'}
                        </h2>
                        <p className={`font-display font-bold ${getSize(content.titleLarge?.fontSize, 'text-5xl')}`}>
                            {content.titleLarge?.content || 'PORTFOLYO'}
                        </p>
                    </motion.div>
